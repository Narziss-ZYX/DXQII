...
//int16_t  YO;
    ...
    Translate3D(gMAT,-4,-4,-4); 	//平移变换矩阵,当为边长的一半的倒数时，图像绕着自己旋转，即原点在中心
    Scale_3D(gMAT,4,4,4);		//比例变换矩阵
    Rotate_3D(gMAT,x,y,z);		//旋转变换矩阵
    Translate3D(gMAT,XO,YO,32); 	//平移变换矩阵	   x:调节距离中心点的位置，相当于下面Point0.z
    //		y：上下调动位置 ，具体根据实际修改
//	XO=128;		//128  正中
        ...
        for (i = 0; i < 4; i++) {
            for (k = 0; k < 8; k++) {
                ...
                if (*(dp + i) & temp) {
                    ...
                    PointDis = PerProject(Point1,XO,YO);
                    GUI_DrawPoint(PointDis.x, PointDis.y, color);
                    printf("x:%d,y:%d\r\n",PointDis.x,PointDis.y);
                }
            }
        }
        dp += 4;
